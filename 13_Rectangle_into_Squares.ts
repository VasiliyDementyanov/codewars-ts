...
export const sqInRect = (l: number, w: number): null | number[] => {
    ...
    const resArr: number[] = [];
    const wholeSquare: number = l * w;
    let remainSquare: number = wholeSquare;

    let tL = l;
    let tW = w;
    while (remainSquare > 0) {
        const minSide = Math.min(tL, tW);
        resArr.push(minSide);
        remainSquare -= minSide ** 2;

        let ttL = tL;
        let ttW = tW;
        tL = ttL > ttW ? ttL - ttW : ttL;
        tW = ttW > ttL ? ttW - ttL : ttW;
    }
